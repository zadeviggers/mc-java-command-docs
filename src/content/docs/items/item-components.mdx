...
## Entity variant components <Badge text="Since 1.22?" variant="tip" size="large" />
...
### List of entity variant components
...
See above for a list of entity variant components.
...
### `minecraft:potion_contents` <Badge text="Changed in 1.21.2" variant="caution" />

- Holds the contents of a potion (Potion, Splash Potion, Lingering Potion), or potion applied to an item (Tipped Arrow)
- Can be applied by eating an item with the [`minecraft:consumable` component](#minecraftconsumable)
- Duration can be scaled with the [`minecraft:potion_duration_scale` component](#minecraftpotion_duration_scale)
- Format: object with fields
...
- Format: object with fields
  ...
  - `can_destroy_blocks_in_creative` (optional): boolean
...
### `minecraft:equippable` <Badge text="Since 1.21.2" variant="tip" /> <Badge text="Changed in 1.22?" variant="caution" />

- If present, this item can be equipped by players in the selected slot
- Any mob can have equipment added in the `body` slot as long as the component allows it (`allowed_entities`)
- Format: object with fields
  - `slot`: one of `saddle`, `head`, `chest`, `legs`, `feet`, `body`, `mainhand`, or `offhand`
  ...
  - `equip_on_interact`: boolean (default: `false`)
